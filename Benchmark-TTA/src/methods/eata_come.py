...
class EATA_COME(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, num_classes:int = 1000, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.model, self.optimizer, self.fishers, self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin, num_classes=self.num_classes)
...
@torch.jit.script
def dirichlet_entropy(x, num_classes:int):#key component of COME
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, num_classes:int=1000, scale_factor=2, num_samples_update=0):
    ...
    entropys = dirichlet_entropy(outputs, num_classes=num_classes)
